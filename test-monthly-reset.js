...
async function testMonthlyReset() {
  try {
    ...
    await mongoose.connect(process.env.MONGODB_URI || 'mongodb://localhost:27017/codehub-test');
    ...
    const beforeReset = await User.find({ email: { $in: testUsers.map(u => u.email) } });
    console.log('\nUsers BEFORE monthly reset:');
    beforeReset.forEach(user => {
      console.log(`${user.email} (${user.subscriptionPlan}): Chat=${user.chatQueriesRemaining}, Code=${user.codeQueriesRemaining}, Tutorial=${user.tutorialGenRemaining}`);
    });
    ...
    const afterReset = await User.find({ email: { $in: testUsers.map(u => u.email) } });
    ...
    const premiumUsers = afterReset.filter(u => u.subscriptionPlan === 'premium');

    console.log('\nVerification:');
    
    // Check free users have 5 queries each
    const correctFreeUsers = freeUsers.every(user => 
      user.chatQueriesRemaining === 5 && 
      user.codeQueriesRemaining === 5 && 
      user.tutorialGenRemaining === 5
    );
    console.log(`✓ Free users reset to 5 queries: ${correctFreeUsers ? 'PASS' : 'FAIL'}`);

    // Check premium users kept their limits
    const correctPremiumUsers = premiumUsers.every(user =>
      user.chatQueriesRemaining === 9999 &&
      user.codeQueriesRemaining === 9999 &&
      user.tutorialGenRemaining === 9999
    );
    console.log(`✓ Premium users kept unlimited queries: ${correctPremiumUsers ? 'PASS' : 'FAIL'}`);
