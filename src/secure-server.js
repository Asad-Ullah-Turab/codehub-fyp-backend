#!/usr/bin/env node
/**
 * Enhanced Secure Startup Script for CodeHub Backend
...
class SecureContainerManager {
  constructor(originalManager) {
    ...
    this.securityConfigs = {
      python: {
        secure: "codehub-python-secure",
        fallback: "codehub-python-fallback",
        original: "codehub-python-persistent",
      },
      javascript: {
        secure: "codehub-javascript-secure",
        fallback: "codehub-javascript-fallback",
        original: "codehub-javascript-persistent",
      },
      cpp: {
        secure: "codehub-cpp-secure",
        fallback: "codehub-cpp-fallback",
        original: "codehub-cpp-persistent",
      },
    };
  }
  ...
  async selectBestImage(language) {
    ...
    for (const [type, imageName] of Object.entries(configs)) {
      ...
      if (exists) {
        console.log(
          `✅ Using ${type} configuration for ${language}: ${imageName}`,
        );
  ...
  async startSecureContainers() {
    console.log("\n🔒 Starting CodeHub with Security Enhancements");
    console.log("================================================");
    ...
    for (const language of languages) {
      try {
        ...
        if (imageConfig) {
          console.log(
            `📦 Image available: ${imageConfig.imageName} (${imageConfig.type})`,
          );
...
async function startSecureServer() {
  try {
    ...
    server.listen(PORT, "0.0.0.0", () => {
      console.log("\n✅ CodeHub Backend Started Successfully!");
      console.log("========================================");
      ...
      console.log(
        "   • docker-compose.fallback.yml - Fallback secure configuration",
      );
      console.log("   • Dockerfile.*.secure - Secure container definitions");
      console.log(
        "   • docs/Docker_Security_Guide.md - Security documentation",
      );
...
process.on("unhandledRejection", (reason, promise) => {
  console.error(
    "🚨 SECURITY ALERT - Unhandled Rejection at:",
    promise,
    "reason:",
    reason,
  );
