...
export const signup = async (req, res) => {
  try {
    ...
    if (existingUser) {
      ...
      if (!existingUser.isEmailVerified && existingUser.accountStatus === 'pending') {
        ...
        if (emailService.isAvailable()) {
          try {
            await emailService.sendVerificationOTP(email, otp, existingUser.name);
    ...
    if (emailService.isAvailable()) {
      ...
      console.log('⚠️  Email service not available - auto-verifying user for development');
...
export const verifyEmail = async (req, res) => {
  try {
    ...
    console.log('📧 Email verification attempt:', { email, otp: otp ? '****' : 'missing' });
    ...
    if (!user.emailVerificationOTP || !user.emailVerificationOTPExpires) {
      return res.status(400).json({
        status: 'fail',
        message: 'No pending verification found for this email. Please request a new verification code.'
      });
    }
    ...
    if (!user.isEmailVerified || user.accountStatus === 'pending') {
      ...
      if (emailService.isAvailable()) {
        try {
          ...
          return res.status(401).json({
            status: 'fail',
            message: 'Please verify your email before signing in. Failed to send verification email.',
            needsEmailVerification: true,
            email: user.email,
            autoResent: false
          });
...
export const resetPassword = async (req, res) => {
  try {
    ...
    let decoded;
    try {
      decoded = jwt.verify(resetToken, process.env.JWT_SECRET || 'your-super-secret-jwt-key');
