...
class CodeExecutorService {
  ...
  async runInDocker(code, language, sessionDir, input) {
    ...
    try {
      ...
      await execAsync(
        `docker build -t ${imageName} -f ${dockerfilePath} ${sessionDir}`,
        {
          timeout: 30000,
        },
      );
      ...
      if (error.message.includes("timeout") || error.code === "ETIMEDOUT") {
        return {
          output:
            "Error: Code execution timed out (10 second limit). Your code may have an infinite loop or is taking too long to execute.",
          error: true,
