...
const seedTutorials = async () => {
  try {
    // Connect to MongoDB
    await mongoose.connect(process.env.DB_URI || 'mongodb://localhost:27017/codehub', {
    ...
    // Count by language
    const pythonCount = await Tutorial.countDocuments({ language: 'python', isPreGenerated: true });
    const cppCount = await Tutorial.countDocuments({ language: 'cpp', isPreGenerated: true });
    const jsCount = await Tutorial.countDocuments({ language: 'javascript', isPreGenerated: true });
    ...
    const pythonConcepts = await Tutorial.distinct('concept', { language: 'python', isPreGenerated: true });
    const cppConcepts = await Tutorial.distinct('concept', { language: 'cpp', isPreGenerated: true });
    const jsConcepts = await Tutorial.distinct('concept', { language: 'javascript', isPreGenerated: true });
